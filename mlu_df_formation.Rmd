...
```{R}
  data <- list.files(path = "/Users/esbenkam/Google Drev/Skole/UNI/R/MLU/MLU/data", full.names = TRUE)
``` 
...
```{R}
df <- select(sarah_data, "Speaker", "Gloss", "Date")
```
...
```{R}
?write.csv
write.csv(df, file = "sarah_data.csv")
```
...
```{R}
sarah_data <- read.csv("sarah_data.csv", header = TRUE)
```
...
?as.Date
sarah_data$Date <- as.Date(sarah_data$Date, format = "%d-%b-%Y")
```
...
```{R}
df <- subset(sarah_data, Speaker == "MOT" | Speaker == "CHI")
View(df_mot_chi)
?subset
```
...
```{R}
?str_replace_all
df$clean <- str_replace_all(df_mot_chi$Gloss, "[:punct:]", "")
```
...
3.2) The function "stri_count_words" from the package "stringi" does excatly what you would expect it to do. First install/load "stringi". Secondly, create a new column using "stri_count_words" to count the number of words in each utterance. 
```{R} 
?stri_count_words
df$Count <- stri_count_words(df$clean)
...
df_group <- group_by(df, Speaker, Date)
...
mlu_df <- summarise(df_group, word_sum = sum(Count),
                        utterances = length(clean),
...
```{R}
write.csv(df_group, file = "df_group.csv")
write.csv(mlu_df, file = "mlu_df.csv")
```
